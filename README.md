...
- Unity Asset Store 로컬 캐시(`.unitypackage`)도 자동 임포트 (M7 예정)
...
| **MCP 17 도구** | `find_asset`, `suggest_packs`, `record_asset_use`, `request_user_pick` 등 |
...
| **M5 — 웹 GUI 전환 + Claude pick** | **✅ 완료** | FastAPI 웹 UI + Qt 폐기 + 17 MCP 도구 |
| M6 — 시트 분석 + 애니메이션 | 대기 | 격자 분할, `suggest_animation_frames` |
| M7 — Unity Asset Store 임포트 | 대기 | `.unitypackage` 파서 |
| M8 — 패키징 + i18n | 대기 | PyInstaller, gettext |
...
→ `796 passed, 1 skipped, 4 deselected` 가 나오면 준비 완료.
...
| [`docs/MCP_USAGE_GUIDE.md`](./docs/MCP_USAGE_GUIDE.md) | Claude Code — MCP 17 도구 사용 예시 |
