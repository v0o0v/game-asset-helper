# Game Asset Helper 설계 문서

## 1. 개요

Game Asset Helper(이하 **GAH**)는 Unity 게임 개발 중 Claude Code(또는 다른 MCP 클라이언트)가 "필요한 에셋"을 자연어로 요청하면 로컬에 보관된 에셋 라이브러리에서 가장 적합한 것을 찾아 돌려주는 **MCP 서버 + 트레이 상주 데스크톱 앱**이다.
...
- **Unity Asset Store 로컬 캐시 자동 임포트** — 사용자가 Unity Hub/Editor로 이미 받은 `.unitypackage` 파일을 추출해 GAH 라이브러리에 팩으로 자동 등록
...
```
+---------------------- Game Asset Helper (Python) ----------------------+
|                                                                        |
|  +---------------+      +-----------------+      +------------------+  |
|  | Tray GUI App  |<---->|  Core Service   |<---->|  MCP Server      |  |
|  | (PySide6)     | IPC  |  (asyncio)      | API  |  (stdio/SSE)     |  |
|  +---------------+      +--------+--------+      +------------------+  |
|                                  |                                     |
|              +-------------------+--------------------+                |
|              |                   |                    |                |
...
- **Tray GUI 스레드** — PySide6의 `QApplication` 이벤트 루프. 트레이 아이콘과 메인 윈도우 렌더링.
...
MCP 클라이언트(Claude Code)와의 통신은 **stdio** 가 기본이지만, 트레이 앱이 떠 있는 동안에는 같은 메타데이터 DB를 공유하는 **로컬 HTTP/SSE 엔드포인트**도 같이 노출한다. Claude Code 설정 파일에는 stdio 명령(`game-asset-helper --mcp`)을 등록하면 자식 프로세스가 GUI 인스턴스의 DB를 lock-free로 읽도록 한다(WAL 모드).
...
- 감시 대상: `%APPDATA%/GameAssetHelper/library/`
- 동작
...
- 위치: `%APPDATA%/GameAssetHelper/metadata.db`
...
- 두 가지 실행 모드
  - **stdio 모드** — `game-asset-helper --mcp` 호출 시 동작. Claude Code가 자식 프로세스로 띄움.
  - **HTTP/SSE 모드** — GUI 앱이 떠 있을 때 `127.0.0.1:9874`에 노출 (포트는 설정 가능). 같은 머신의 다른 클라이언트가 붙을 수 있게.
- 두 모드 모두 같은 DB 파일을 본다. stdio 모드는 GUI가 안 떠 있어도 동작해야 하므로 자체적으로 워처를 띄울지는 옵션(`--passive` 플래그)으로 결정한다. 기본은 GUI 인스턴스가 워처를 소유하고 stdio 인스턴스는 read-only.
...
- 프레임워크: PySide6
- 기본 동작: 창을 닫아도 종료되지 않고 트레이로 들어간다.
- 트레이 메뉴: `메인 창 열기`, `라이브러리 폴더 열기`, `재스캔`, `Ollama 상태 확인`, `종료`.
- 메인 창 탭 구성
  - **팩** — 등록된 팩 목록, 각 팩의 집계 메타(파일 수·주 스타일·팔레트), 매니페스트 편집, 강제 재인덱싱, 팩 단위 enable/disable. 팩을 비활성화하면 검색 후보에서 제외.
  - **라이브러리** — 썸네일 그리드, 좌측 필터(팩/타입/카테고리/태그), 상단 검색 박스.
  - **상세** — 선택한 에셋의 메타데이터 보기/수정. Gemma 4가 채운 필드를 사용자가 덮어쓸 수 있고, 그 경우 `manual_override=true`로 기록해서 재분석이 덮어쓰지 못하게 한다.
  - **프로젝트** — 등록된 프로젝트 목록과 각 프로젝트가 사용 중인 팩 분포, 통일성 점수 가시화. 프로젝트별로 `pinned_pack_id`(항상 우선)와 `blocked_pack_ids`를 지정할 수 있다.
  - **Unity Asset Store** — 감지된 캐시 경로, 발견된 `.unitypackage` 수, 마지막 동기화 시각, "지금 동기화" 버튼, 자동 동기화 토글(매일 1회/시작 시), 옵트인 비공식 경로 활성화 스위치(경고 동반).
  - **설정** — 라이브러리 폴더 경로, Ollama 엔드포인트/모델명, 자동 실행, MCP 포트, 통일성 가중치 슬라이더, Unity Asset Store 캐시 경로 오버라이드.
  - **로그** — 분석 큐 상태, 최근 에러.
- 윈도우 시작 시 자동 실행
  - 레지스트리 `HKCU\Software\Microsoft\Windows\CurrentVersion\Run`에 `GameAssetHelper` 키 등록/해제 토글.
  - 시작 인자에 `--tray` 를 붙여 창이 안 뜨고 바로 트레이로 가게.
...
- **캐시 위치**: 기본 `%APPDATA%\Unity\Asset Store-5.x\` ([Unity 매뉴얼](https://docs.unity3d.com/Manual/upm-config-cache-as.html)). 사용자가 Unity Preferences에서 변경했거나 `ASSETSTORE_CACHE_PATH` 환경변수를 설정한 경우 그 값을 우선 사용한다. 두 경로가 모두 비어 있으면 GAH 설정 화면에서 사용자에게 직접 경로를 묻는다.
...
사용자가 GAH가 제공하는 `.unitypackage`(작은 Editor 스크립트 + 메뉴 항목) 하나를 Unity 프로젝트에 임포트하면, "Tools → Game Asset Helper → Export My Assets" 메뉴로 Unity의 `AssetDatabase`/`PackageManager` API를 통해 보유 에셋을 GAH 라이브러리로 안전하게 내보내는 흐름. 공식 API만 쓰므로 가장 신뢰성이 높지만, 사용자가 Unity를 열고 한 번 임포트해야 하는 수동 부담이 있다. v1에는 포함하지 않고 v2 검토.
...
Claude Code가 "이 에셋이 잘 안 맞았다"는 신호를 보낼 수 있게 한다. 추후 검색 가중치 학습용으로 누적 저장(v1에서는 단순 로그 + 해당 `query_id`의 1순위를 향후 페널티 대상으로 기록).
...
│       ├── core/
│       │   ├── watcher.py
│       │   ├── pack_manager.py   # 팩 인테이크, 매니페스트, 집계 메타
│       │   ├── analyzer/
│       │   │   ├── base.py
│       │   │   ├── sprite.py
│       │   │   ├── spritesheet.py
...
│       │   ├── store.py          # SQLite 래퍼
│       │   ├── search.py         # FTS + 벡터 결합
│       │   ├── consistency.py    # 통일성 스코어러
│       │   ├── usage_tracker.py  # 명시·암묵 사용 이력
│       │   └── unity_import/
│       │       ├── cache_scanner.py    # %APPDATA% Asset Store-5.x 스캔
│       │       ├── unitypackage.py     # .unitypackage tar 추출기
│       │       └── remote_optin.py     # 옵트인 publisher 패널 클라이언트
│       ├── mcp/
│       │   ├── server.py         # MCP SDK 바인딩
│       │   └── tools.py          # find_asset 등 도구 구현
│       ├── platform/
│       │   ├── autostart_win.py  # 레지스트리 Run 키
│       │   └── single_instance.py
│       └── config.py
└── tests/
    ├── fixtures/                 # 테스트용 작은 PNG/WAV
...
- 로그(`gah.log`)는 회전 핸들러로 10MB × 5개 한정.
- 단일 인스턴스 보장: `%APPDATA%/GameAssetHelper/gah.lock` 파일 lock으로 중복 실행 방지. 두 번째 실행은 트레이의 기존 인스턴스에게 "show window" IPC만 보내고 종료.
