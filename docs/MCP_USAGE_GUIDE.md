...
이 문서는 **Claude Code(또는 다른 MCP 클라이언트)가 AssetCacheMCP MCP 서버를 어떻게 사용하면 좋은지** 알려 주는 가이드. M3 가 MCP stdio 서버를 구현하면서 채워 졌다.
...
AssetCacheMCP 의 라벨 어휘는 24축 ≈ 316개의 영어 enum 토큰으로 구성된다. 각 라벨은 영어 한 줄 description 을 동봉하므로, Claude Code 는 사용자 자연어 쿼리를 **라벨 어휘에 매핑**할 수 있다.
...
AssetCacheMCP 웹 UI 가 열려 있어야 동작한다.
...
| 코드 | 의미 |
|---|---|
| `503_no_ui_available` | AssetCacheMCP 트레이 앱 미실행 또는 웹 포트 미기록 |
...
| `503_no_ui_available` | AssetCacheMCP 웹 UI 미실행 또는 포트 미기록 | M5 `request_user_pick` |
...
3. **AssetCacheMCP 웹 UI 가 열려 있을 때** — 트레이 앱이 실행 중이고 `web.port` 파일이 존재해야 한다.
...
AssetCacheMCP 웹 UI 의 라이브러리 페이지 상단에 **보라색 줄무늬 카드 그룹** 이 나타난다. 각 카드에는 후보 에셋 이름, 팩, 크기, 썸네일이 표시된다.
...
트레이 아이콘 툴팁에는 **"AssetCacheMCP — Claude 요청 N건"** 이 표시되어 사용자가 대기 중인 요청 수를 알 수 있다.

### 12.4 실패 시나리오별 폴백 전략

| 에러 코드 | 원인 | 권장 폴백 |
|---|---|---|
| `503_no_ui_available` | AssetCacheMCP 트레이 앱 미실행 또는 포트 파일 없음 | 자동 top-1 픽 + 사용자에게 AssetCacheMCP 앱 실행 안내 |
...
Claude:
  ...
  3. 결과를 사용자에게 안내:
     "Kenney Character Pack (42 이미지, 3 사운드)를 발견했어요.
      아직 AssetCacheMCP 라이브러리에 임포트되지 않았습니다.
