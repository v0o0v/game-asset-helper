...
def test_register_all_tools_count_is_20(mcp_tool_deps) -> None:
    ...
    # 또는 `list_tools()` 어느 쪽이든 길이 20 이어야.
    names: set[str] = set()
    ...
    assert len(names) == 20
    # M4 4 신규 도구 + M5 1 신규 도구 + M6 1 신규 도구 + M7 2 신규 도구 모두 등록되었는지 확인.
    assert {"save_search", "list_saved_searches",
            "delete_saved_search", "run_saved_search",
            "request_user_pick", "suggest_animation_frames",
            "scan_unity_asset_store_cache", "list_unity_packages"} <= names
