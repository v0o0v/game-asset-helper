...
def test_register_all_tools_count_is_18(mcp_tool_deps) -> None:
    """M3 의 12 → M4 의 16 → M5 의 17 → M6 의 18 (suggest_animation_frames 추가)."""
    ...
    # 또는 `list_tools()` 어느 쪽이든 길이 18 이어야.
    names: set[str] = set()
    ...
    assert len(names) == 18
    # M4 4 신규 도구 + M5 1 신규 도구 + M6 1 신규 도구 모두 등록되었는지 확인.
    assert {"save_search", "list_saved_searches",
            "delete_saved_search", "run_saved_search",
            "request_user_pick", "suggest_animation_frames"} <= names
