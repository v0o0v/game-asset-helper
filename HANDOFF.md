...
**마지막 인계 시각**: 2026-05-23 (M11.10 [PR #35](https://github.com/v0o0v/assetcache-mcp/pull/35) main squash 머지 `e986032`)
**마지막 완료 작업**: **M11.10 — text_embed multi-input batch + race window 0 + batch-only 정책** — LIVE Gemini API 호출 26 sprite 재분석 **53 → 2 (26× 감소)**.  핵심 산출: `BackendChain.batch_embed` + `GeminiBackend.embed_multi` + `BatchPoller._embed_chat_results` + atomic `try_mark_asset_analyzing` + drain_pending 순서 fix + `detect_sheet` 1D strip ratio fallback + thumbnail cache key file_hash + batch-only 정책 (toggle/threshold/polling 사용자 설정 제거) + 트레이 "라이브러리 폴더 열기" UX.  회귀 **1594 passed + 3 skipped + 57 deselected**.  v0.2.9 candidate (publish 보류, v0.2.3~v0.2.9 누적).  ⚠️ **다음 세션 우선 작업**: **M11.11 — 2D grid sheet detection 강화** — M11.10 LIVE 한계 (Female1-Male3 256×576, Warrior_Sheet-* 414×748 같은 2D grid 시트가 sprite 분류).  spec: [2026-05-23-m11-11-2d-grid-sheet-detection](./docs/superpowers/specs/2026-05-23-m11-11-2d-grid-sheet-detection.md) / plan: [M11_11_plan.md](./milestones/M11_11_plan.md).
